...
| Stripe | Checkout de cartao e webhooks | E-mail, customer, assinatura, status de pagamento; cartao fica no Stripe | Execucao de contrato e pagamento | `src/functions/stripe-checkout.functions.ts`, `src/routes/api/public/stripe-webhook.ts` | Necessario para pagamento por cartao | Webhook secret, checkout hospedado, sem cartao local |
| Asaas | Pix, Pix Automatico, clientes e transferencias Pix | Nome, e-mail, CPF/Pix, valor, status de pagamento/transferencia | Execucao de contrato, repasse e cobranca | `src/server/asaas.server.ts`, `src/routes/api/public/asaas-webhook.ts` | Necessario para Pix/saques | Webhook token, env vars na Vercel, idempotencia |
...
- Webhooks devem continuar usando assinatura ou token: Stripe usa `STRIPE_WEBHOOK_SECRET`; Asaas usa `ASAAS_WEBHOOK_TOKEN`.
