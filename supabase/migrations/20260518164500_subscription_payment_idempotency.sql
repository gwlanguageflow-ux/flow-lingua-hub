...
UPDATE public.teacher_wallet_transactions
SET subscription_payment_reference = subscription_id::text
WHERE subscription_id IS NOT NULL
  AND transaction_type = 'manual_adjustment'
  AND subscription_payment_reference IS NULL;

UPDATE public.platform_wallet_transactions
SET subscription_payment_reference = subscription_id::text
WHERE subscription_id IS NOT NULL
  AND transaction_type = 'subscription_fee'
  AND subscription_payment_reference IS NULL;

DROP INDEX IF EXISTS idx_teacher_wallet_subscription_credit_once;
DROP INDEX IF EXISTS idx_platform_wallet_subscription_fee_once;

CREATE UNIQUE INDEX IF NOT EXISTS idx_teacher_wallet_subscription_payment_once
  ON public.teacher_wallet_transactions (subscription_id, subscription_payment_reference)
  WHERE transaction_type = 'manual_adjustment'
    AND subscription_id IS NOT NULL
    AND subscription_payment_reference IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS idx_platform_wallet_subscription_payment_once
  ON public.platform_wallet_transactions (subscription_id, subscription_payment_reference)
...
BEGIN
  ...
  VALUES (
    ...
    _gross,
    0.1000,
    'Taxa da plataforma sobre assinatura paga',
    _sub.student_id
  )
  ON CONFLICT (subscription_id, subscription_payment_reference)
  WHERE transaction_type = 'subscription_fee'
...
REVOKE EXECUTE ON FUNCTION public.activate_paid_student_subscription(uuid, text, timestamptz, timestamptz, text)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.activate_paid_student_subscription(uuid, text, timestamptz, timestamptz, text)
  TO service_role;
